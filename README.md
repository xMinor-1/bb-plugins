...
| [file-manager](plugins/file-manager) | Файловый менеджер на весь экран, пункт «File Manager» в сайдбаре, он же вкладкой правой панели рядом с тредом («New tab» → Actions), и «Open with File location» по правому клику на ссылке файла в сообщении — открывает папку файла с выделенным файлом, а для неверного пути (глоб, переименованный файл) ближайшую существующую папку с подставленным фильтром. Дерево с раскрытием папок на месте, адресная строка для вставки пути, возврат в последнюю открытую папку, загрузка перетаскиванием кусками с докачкой после обрыва (проверено на 5 ГБ), скачивание потоком, перемещение, копирование, переименование, удаление, распаковка архивов; доступ ограничен домашней папкой пользователя, под которым работает bb | Релиз `file-manager/v0.6.2`; в каталоге bb ([PR #90](https://github.com/get-bb/marketplace/pull/90)), запись обновляется на 0.6.x в [PR #122](https://github.com/get-bb/marketplace/pull/122) |
...
bb plugin install git:https://github.com/xMinor-1/bb-plugins.git@^0.6.0 --plugin file-manager --tag-prefix file-manager/
