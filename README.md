...
bb plugin install git:https://github.com/xMinor-1/bb-plugins.git@main --plugin usage-meter
```

Первые три формы ставят последний релиз по тегу `<плагин>/vX.Y.Z` и видят
обновления через `bb plugin outdated`; `@main` берёт текущее состояние ветки —
так ставится то, что ещё не выпущено релизом.
