...
| --- | --- | --- |
| [theme-toggle](plugins/theme-toggle) | Кнопка темы в футере сайдбара: клик — переключение день / ночь, наведение с задержкой, удержание или правый клик — меню выбора палитры и режима светлая / тёмная / как в системе. Иконка — квадрат, разрезанный по диагонали из правого верхнего угла в левый нижний: верх день, низ ночь, залитая половина показывает текущую тему; размер вровень с кольцами соседних плагинов | В каталоге bb релиз `theme-toggle/v0.2.2`; имя автора правится — [PR #110](https://github.com/get-bb/marketplace/pull/110). Версия 0.3.0 (клик = день / ночь, новая иконка) ещё не выпущена тегом, для каталога нужен новый PR: диапазон `^0.2.0` её не покрывает |
...
bb plugin install git:https://github.com/xMinor-1/bb-plugins.git@^0.4.0 --plugin file-manager --tag-prefix file-manager/
