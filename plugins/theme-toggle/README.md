...
The button wears a square cut from the top-right corner to the bottom-left
one: day above the cut, night below it. The half that is on is filled and holds
its symbol punched through it, the other half is an empty outline with its
symbol drawn small — so the fill moves from one half to the other on every
flip, and the button shows the state it is in, not only the one it would move
to. The square is drawn larger than the host's 16 px icon slot, level with the
rings other footer plugins paint around their buttons.
...
Three things the artwork depends on. A mask reads alpha, so the symbol on the
filled half is cut out through an SVG `mask` rather than painted in a lighter
colour. The drawing uses the whole canvas, since bb fits the entire viewBox
into the icon box and empty margin in the file is size the icon loses. And the
span is scaled with a transform rather than resized, so the bigger square never
reflows the footer row around it.
